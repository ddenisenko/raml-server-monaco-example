...
export class EditorManager {
    ...
    constructor(display: boolean = true) {
        ...
        this.markOccurrencesReconciler = new ramlServer.Reconciler(ramlServer.getConnection(), 200);
        ...
        atom.workspace.observeTextEditors(editor=>{
            editor.onDidDestroy(()=>{
                ...
                ramlServer.getConnection().documentClosed(path);
            })
    ...
    private cursorChanged(editor: atom.TextEditor, newBufferPosition: atom.Point) {
        ...
        ramlServer.getConnection().positionChanged(manager.unitPath, pos);
    ...
    private addListenersForStructure() {
        ramlServer.getConnection().onStructureReport(report=>{

            let categoryNames = []
            for(let categoryName in report.structure) categoryNames.push(categoryName);

            let categoryNamesString = categoryNames.join();
            ramlServer.getConnection().debug("Got new structure report with categories "
    ...
    updateOutline() {
        ramlServer.getConnection().debug("Updating outline", "EditorManager", "updateOutline");
    ...
    positionUpdated(newPosition) {
        ...
        if (this._details) {
            ramlServer.getConnection().positionChanged(manager.unitPath, this.currentPosition);
        }
    }
